...
const multipleMeasurementsQuery = `query ($input: [MeasurementQuery]) {
...
const MetricOption = ({
  ...
  handleClose,
  handleOpen,
  selectedValue = "tubingPressure",
  handleChange,
  options = ["tubingPressure"]
}) => {
  return (
...
class App extends React.Component {
  ...
  componentDidMount() {
    fetchDataFromGraphqlApi(metricsQuery, {}).then(
      ({
        data: {
          data: { getMetrics: metricValues }
        }
      }) => {
  ...
  fetchDataOnChange = (metricName = "tubingPressure") => {
    ...
    fetchDataFromGraphqlApi(multipleMeasurementsQuery, inputVariables).then(
      ({
        data: {
